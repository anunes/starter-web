...
class Database
{
    ...
    public function __construct()
    {
        ...
        if ($type === 'sqlite') {
            if ($database === '') {
                throw new Exception('SQLite database path is required');
            }
    ...
    public function rows(string $sql, array $args = [], int|object $fetchMode = PDO::FETCH_OBJ): array
    {
    ...
    public function row(string $sql, array $args = [], int|object $fetchMode = PDO::FETCH_OBJ): ?object
    {
        $result = $this->run($sql, $args)->fetch($fetchMode);
    ...
    public function getById(string $table, int $id, int|object $fetchMode = PDO::FETCH_OBJ): ?object
    {
        $result = $this->run("SELECT * FROM $table WHERE id = ?", [$id])->fetch($fetchMode);
        return $result === false ? null : $result;
    }
